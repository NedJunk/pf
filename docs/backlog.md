...
- [ ] **BUG-05 — router produces unsolicited closings and summaries** — contract fix applied (2026-04-29, commit 743db2b). Residue observed in session 8d775b26: router produced three consecutive closing-question turns ("Are there any outstanding questions... before you conclude?" / "or tasks you'd like to address before concluding?" / "before concluding?") before finally landing on "Understood." The fragmentation suggests BUG-07 (stutter) is amplifying the closing pattern. May resolve when BUG-07 is fixed; re-evaluate after.
...
- [x] **BUG-07 — duplicate/fragmented assistant turns in transcript** — fixed (2026-04-29, commit efd7aac). Root cause: Gemini Live API emits multiple consecutive `turn_complete` events per logical response. Fix: `_flush_output_buf()` coalesces into any trailing `"Assistant:"` entry. **Separator fix applied** (commit 3be9329): run-on joins like `?Are` now get a space inserted — confirmed effective in session 6275d9ca (no run-ons of that form). **Fragmentation persists** via a different mechanism (session 6275d9ca, 2026-04-29): when a user `turn_complete` fires mid-assistant audio stream, chunks that should form one response land as separate history entries (e.g. `"incorrectly speaks"` / `"aloud hidden system messages."` split across two turns). This is a race condition between audio transcription delivery and user turn events — distinct from the multi-flush coalescing issue. **Fix direction:** detect output_transcription chunks arriving after a user `turn_complete` in the debug logs (committed in 3be9329); signal to look for is `branch=user output_buf>0`. User confirmed threading hypothesis during session 6275d9ca.

- [ ] **BUG-10 — router vocalizes whisper content** — confirmed in session d5920363 (2026-04-29): router spoke `[WHISPER from insight_engine]: The current debug mode is still in design...` verbatim, including the injection prefix. **Confirmed again with multiple instances in session 6275d9ca (2026-04-29):** router answered DevCoach whispers directly (passphrase suggestion, transcript-capture prompt) and partially read a raw injection prefix aloud: `[WHISPER fromDevCoach]: The` — note the malformed space drop (`fromDevCoach` vs `from DevCoach`), which may indicate Gemini's transcription is mangling the injected text, or Gemini is generating it from context. A DevCoach-vocalizes-BUG-10 feedback loop was observed: each vocalization prompted a new DevCoach whisper, which prompted another vocalization. Whispers are injected via `send_realtime_input(text=...)` and must remain silent context — the router should never repeat or acknowledge whisper text in spoken output. Distinct from BUG-06 (denying awareness). Behavioral contract change required; debug logs from session 6275d9ca should clarify the malformed-prefix origin.

- [ ] **BUG-11 — multiple ingest calls per session** — session 6275d9ca fired 3 ingest calls vs the expected 1. Investigate whether `LiveSession.close()` `_closed` guard prevents duplicate transcript writes but not duplicate `/sessions/{id}/close` POSTs to the orchestrator. Also check for session reconnect events that might trigger additional close paths.
