...
- [x] **E4-E — Design: agent routing improvements (Fix Fan-Out Problem)** — delivered (2026-05-05): see Epic 4 entry for full detail. Design doc at `docs/specs/orchestrator-routing-design.md`.
...
- [x] **BUG-21 — whisper delivery rate dropped to 37.5%** — session 1f89e5e2 (2026-05-04) recorded 24 acks but only 9 deliveries (37.5%), down from 91.7% the prior session; 15 acknowledged dispatches had no delivery callback. **Update (session 7a263bea, 2026-05-04):** delivery was 16/16 (100%) in a 16-turn session — may be load-related or incidentally fixed by BUG-03 connection pooling. **Confirmed fixed (session c1131b3f, 2026-05-05):** 25+ turn session with continuous DevCoach whisper activity throughout; log metrics unavailable (containers stopped at review time) but transcript evidence confirms sustained delivery. Closed.
...
- [x] **BUG-24 — DevCoach whispers reference hallucinated personas and invented patterns** — session 7a263bea (2026-05-04): DevCoach suggested "addressing the Project Manager persona", using "Live Timestamp Isolation", "Logging Extraction", "Validation Passphrase", and "Bug Validation Patterns wiki" — none of which exist in the project. Root cause likely: `ROADMAP_PATH` was not wired in `docker-compose.yml`, so E4-M ran with no backlog context and the model filled the void with hallucinated patterns. Fix: mount backlog and set `ROADMAP_PATH` in dev-coach service config; verify whisper quality in next session. **Update (session c1131b3f, 2026-05-05):** ROADMAP_PATH fix is working — router showed accurate roadmap awareness. However, DevCoach's on-disk wiki still contains hallucinated content written before the fix (non-existent files, invented patterns). Root cause is stale wiki, not the prompt. Will close after BUG-29 wiki audit and one clean whisper session. **Update (sprint 4, 2026-05-05): BUG-29 wiki audit complete — hallucinated content removed. Ready for one post-cleanup live session to confirm DevCoach whispers are grounded before closing.** **Confirmed closed (session 3ec6322f, 2026-05-05):** router output was clean throughout; DevCoach whispers were topically relevant coaching suggestions with no invented personas or patterns. One suspicious whisper referenced an "Intellect Dashboard" — tracked separately as BUG-31.

- [x] **BUG-25 — Router spoke before user's first turn (session opener regression)** — confirmed fixed (session c1131b3f, 2026-05-05): user explicitly stated "I think this session opened correctly. So bug 25 will close." Router waited for user input before generating any audio. Two consecutive post-rebuild sessions with no reproduction — closed.

- [x] **BUG-26 — Router echoes user-provided codes in speech (BUG-18 incomplete)** — fixed (2026-05-05): behavioral contract strengthened to explicitly cover the echo case — even when the user introduces an internal code, Kai must translate to meaning rather than repeat the identifier. Phrase "even when the user introduces one" and "echo" added so the rule is unambiguous. Regression test updated to assert both phrases are present.

- [ ] **BUG-28 — Backlog context injection conflicts with BUG-18/26 prohibition** — session c1131b3f (2026-05-05): router used internal codes (Epic 4-E, E4-F, E2-A, E6-C, E6-C1, E6-C2) in spoken responses without the user introducing them, despite BUG-18/26 fixes. Root cause: `live_session.py` appends "Reference specific items when the user mentions bugs, epics, or work items by code" to the system instruction after the behavioral contract — directly overriding the prohibition. Fix: rewrite the injection instruction to "describe items by their meaning, not by code identifier."

- [ ] **BUG-29 — DevCoach wiki contains hallucinated content from pre-ROADMAP_PATH sessions** — session c1131b3f (2026-05-05): DevCoach whispered references to non-existent files (`patterns-bug-validation.md`, `tech-debug-mode.md`) and invented patterns ("Live Timestamp Isolation", "Initialization Trace Validation", "Cryptic ID Syndrome", "Explicit Persona Addressing") — content written to the wiki during sessions when ROADMAP_PATH was not wired and the model hallucinated freely. Fix: audit `expert-agents/dev-coach/wiki/` and remove or correct all fabricated entries; verify one post-cleanup session before closing BUG-24. **Audit complete (2026-05-05):** deleted `decisions-conversation-parsing.md` (entirely hallucinated: Explicit Persona Addressing, post-session parser with persona routing) and `tech-logging-extraction.md` (entirely hallucinated: "Logging Extraction" as a named feature); removed "Persona Addressing / Project Manager / Transcript Reviewer" bullets from `decisions-backlog-integration.md`, `decisions-section-review-skill.md`, and `patterns-agent-interaction.md`; removed "Cryptic ID Syndrome" as named syndrome and "Orange is as orange does" as deployed passphrase from `problems-initialization-race-condition.md`, `patterns-bug-validation.md`, and `tech-debug-mode.md`; corrected "Coordinator" terminology-shift claim in `decisions-router-architecture.md`; updated broken cross-references caused by deleted files. Remains open until one clean live session confirms whispers are grounded.

- [ ] **BUG-31 — DevCoach wiki contains hallucinated artifact name "Intellect Dashboard"** — session 3ec6322f (2026-05-05): two DevCoach whispers referenced `decisions-debug-dashboard.md` and called the dashboard the "Intellect Dashboard." Neither the name nor that file exists — the actual artifact is `scripts/session-dashboard.sh` (session metric dashboard). The BUG-29 scrub removed fabricated *patterns* and *personas* but may not have caught fabricated *artifact names*. Fix: audit `expert-agents/dev-coach/wiki/` for any remaining references to "Intellect Dashboard" or `decisions-debug-dashboard.md`; correct or remove. Narrowest reproduction: trigger a DevCoach whisper mentioning "dashboard" or "Epic 6" and observe whether the hallucinated name surfaces.
...
- [x] **E4-E — Design: agent routing improvements (Fix Fan-Out Problem)** — delivered (2026-05-05): design doc at `docs/specs/orchestrator-routing-design.md`. Chosen approach: capability tags declared per agent in `agents.yaml`, Jaccard word-overlap scoring in orchestrator, configurable `ROUTING_RELEVANCE_THRESHOLD`, fallback to broadcast-all when no agent scores above threshold. Implementation in E4-F.
...
- [x] **E6-L — Feature: session-review skill accepts optional user observation input** — delivered (2026-05-05): `/session-review` command now parses an optional observation note from `$ARGUMENTS` after the session specifier (ID prefix or type keyword). Observation is stored as `USER_OBSERVATION` and surfaced in the session-summary module under "User observation:". Parsing rules handle all four argument patterns (empty, type, ID prefix, observation-only).

- [x] **E6-K2 — Feature: separate audio-data log gate (`LOG_AUDIO` env var)** — delivered (2026-05-05): `_LOG_AUDIO` module-level flag reads `LOG_AUDIO` env var (default false). Audio byte logs (browser→gemini frame sizes, gemini→browser frame sizes) are gated behind it in `live_session.py`. `LOG_LEVEL=DEBUG` continues to surface event-sequence diagnostics without audio noise. Documented in `docker-compose.yml` comment.

- [ ] **E6-O — Docs: add usage guidance for `scripts/session-dashboard.sh`** — session 3ec6322f (2026-05-05): user asked "how do I look at the debug dashboard" during a live session, indicating the script is not self-discoverable. Add a `--help` output block and a brief usage note in `scripts/session-dashboard.sh` (already has `--help` flag; expand it) and/or a one-liner in `README.md` or `docs/` pointing to the script. Goal: a developer running the stack for the first time can find and start the dashboard without consulting session review transcripts.

- [ ] **E6-N — Design: clarify Router facilitation contract boundaries** — session c1131b3f (2026-05-05): open questions about desired Kai behaviour that need a deliberate design decision before the next behavioral contract revision: (1) when the user asks open-ended questions ("what does your roadmap look like?"), should Kai answer from backlog context or redirect back to the user? (2) when the user pivots topics, should Kai follow, probe the pivot, or hold the thread? (3) given BUG-28, what level of backlog specificity is appropriate in Kai's responses — if codes are stripped entirely, does that reduce utility as a session partner? Deliverable: a brief design decision doc or annotated behavioral contract revision covering these three cases.

- [ ] **E6-M — Feature: real-time session metric dashboard** — session c1131b3f (2026-05-05): user wants a live display of turn count, whisper delivery rate, and TDD validation status during a session as a lightweight in-session quality indicator; scope: read from container log stream and render key metrics in a terminal or browser panel; relates to E6-C1 debug mode design and the TDD philosophy discussed during the session.

- [ ] **BUG-30 — session-review.sh returns all-zero metrics when containers are stopped** — session c1131b3f (2026-05-05): all log metrics (turns, acks, deliveries, ingest) showed 0 despite continuous DevCoach activity visible in the transcript; root cause is containers stopped before review ran, so `docker logs` returned empty output. Fix: detect whether target containers are running before scraping logs and emit a clear warning ("containers not running — metrics unavailable") rather than silently returning zeroes. Distinct from BUG-14 (tail window issue, now fixed).
