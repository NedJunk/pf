...
- [x] **BUG-10 — router vocalizes whisper content** — confirmed in session d5920363 (2026-04-29): router spoke `[WHISPER from insight_engine]: The current debug mode is still in design...` verbatim, including the injection prefix. **Confirmed again with multiple instances in session 6275d9ca (2026-04-29):** router answered DevCoach whispers directly and partially read a raw injection prefix aloud. Root cause: `send_realtime_input(text=...)` is the VAD user-turn channel — Gemini treats any text sent there as a user utterance and immediately generates an audio response. **Fixed (2026-04-29):** switched injection to `send_client_content(turns=..., turn_complete=False)`. `turn_complete=False` tells Gemini to accumulate the whisper context without responding; it incorporates the insight when it next replies to actual user audio. Behavioral contract updated with explicit prohibition on speaking, repeating, or acknowledging whisper content (belt-and-suspenders). **Known risk:** the Gemini SDK (live.py line 188) cautions against interleaving `send_client_content` and `send_realtime_input` in the same session, calling the results "unexpected." We are deliberately doing this — `turn_complete=False` is the only purpose-built silent-injection mechanism available. The fallback if this proves unstable is Option B: buffer whispers and prepend to the next user turn at the orchestrator boundary. Needs a live validation session to close.
